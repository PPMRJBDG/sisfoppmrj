...
{
  $set_modal = '';
  if ($today == date("Y-m-d", strtotime($presence->created_at))) {
    $set_modal = '#presence-modal';
  }
  return "<div data-has-permit=" . ($presence->myPermit() ? '1' : '0') . " class='presence-label btn " . ($presence->myPresent() ? 'btn-success' : ($presence->myPermit() ? 'btn-warning' : ($presence->event_date < date('Y-m-d') ? 'btn-danger' : 'btn-primary'))) . "' data-presence-title='$presence->name' data-present='" . ($presence->myPresent() ? 1 : 0) . "' data-presence-id='$presence->id' data-bs-toggle='modal' data-bs-target='" . $set_modal . "'>$presence->name</div>";
}

function build_calendar($month, $year, $presences, $today)
{
    ...
    if ($date == date("Y-m-d")) {
      ...
      foreach ($presences as $presence) {
        if ($presence->event_date == $date)
          $calendar .= build_presence_label_button($presence, $today);
      }
...
<div class="card">
  ...
  <div class="card-body pt-4 p-3">
    ...
    @endif
    <ul class="list-group">
      @if(sizeof($presences) <= 0) Tidak ada hasil. @endif <div class="calendar-container">
        <?php echo build_calendar($date->format('m'), $date->format('Y'), $presences, $today); ?>
  </div>
  </ul>
  <div class="mt-4">
