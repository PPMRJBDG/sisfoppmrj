...
$("body").on("click", "#close", function (e) {
    ...
    $("#contentReport").html(
        '<tr><td colspan="3"><span class="text-center">Loading...</span></td></tr>'
    );
    $("#alertModal").fadeOut();
    $("#contentAlert").html("");
});
...
function getPage(url) {
    ...
    $.ajax({
        ...
        success: function (data) {
            if (data == "reload") {
                ...
                var include_start =
                    '<div data-mdb-toggle="animation" data-mdb-animation-start="onLoad" data-mdb-animation="fade-in-left">';
                var include_end =
                    "</div>" +
        ...
        error: function () {
            ...
            $("#al-danger").show();
            $("#al-danger").html(
                'Sedang terjadi kesalahan<br><a href="/home" class="btn btn-sm btn-info">Kembali ke Beranda</a>'
            );
        },
    });
    ...
    if (current_urld.match("home") || base_url == current_urld) {
        ...
        current_urld.match("presensi/izin/persetujuan/create") ||
        current_urld.match("/presensi/izin/pengajuan")
    ) {
...
function openTab(data_presence_group) {
    ...
    $.get(
        base_url + `/tabgraf/` + tb + `/` + angkatan + `/` + periode,
        function (data, status) {
            ...
            Object.keys(datax).forEach(function (index) {
                ...
                data_body =
                    ...
                    hari_ini(new Date(index).getDay()) +
                    ", " +
                    formatIndonesia +
                    "</td>";
                var persentase = 0;
                ...
                presence_group.forEach(function (pg_key) {
                    ...
                    if (datax[index][pg_key["id"]] != undefined) {
                        data_body =
                            data_body +
                            '<td class="text-center">' +
                            datax[index][pg_key["id"]]["hadir"] +
                            '</td><td class="text-center">' +
                            datax[index][pg_key["id"]]["ijin"] +
                            '</td><td class="text-center">' +
                            datax[index][pg_key["id"]]["alpha"] +
                            "</td>";
                        hadir =
                            hadir +
                            (datax[index][pg_key["id"]]["hadir"] +
                ...
                var caret =
                    '<i class="fa fa-caret-down text-danger" style="font-size:18px;"><i/>';
                if (prev_persentase < persentase) {
                    caret =
                        '<i class="fa fa-caret-up text-success" style="font-size:18px;"><i/>';
                }
                data_body =
                    data_body +
    ...
    $.get(
        ...
        function (data, status) {
            ...
            presence_group.forEach(function (item, index) {
                var ctx7 = document
                    .getElementById("mixed-chart-" + item.id)
                    .getContext("2d");
                ...
                new Chart(ctx7, {
                    ...
                    data: {
                        labels:
                            data_presensi["tanggal_presensi"][item.id] !=
                            undefined
                                ? data_presensi["tanggal_presensi"][item.id]
                                : "0",
                        datasets: [
                            {
                                ...
                                borderSkipped: false,
                                data:
                                    data_presensi["total_presensi"][item.id] !=
                                    undefined
                                        ? data_presensi["total_presensi"][
                                              item.id
                                          ]["hadir"]
                                        : [0],
                                maxBarThickness: 10,
...
function checkSS(val) {
    ...
    if (val.value.match("Pulang") || val.value.match("Magang")) {
        if (val.value.match("Pulang - Permintaan Ortu")) {
            alert(
                "Perhatian! Sesuai peraturan, diperbolehkan pulang hanya di minggu ke 3 atau 4 saja."
            );
        }
        $("#show-ss").show();
...
function selectAllCheckbox(thisx) {
    ...
    for (var i = 0; i < el.length; i++) {
        ...
        showInputAlasan(
            el[i],
            el[i].getAttribute("presence-id") +
                "-" +
                el[i].getAttribute("santri-id")
        );
    }
}
...
function promptApprovePermit(url, id, presence_id = null, santri_id = null) {
    if (confirm("Yakin di approve ?")) {
        ...
        datax["action"] = "approve";
        datax["data_json"] = JSON.stringify(ival);
        $.get(url, datax, function (data, status) {
            var return_data = JSON.parse(data);
            if (return_data.status) {
                if (return_data.is_present != "") {
                    $("#alertModal").fadeIn();
                    $("#contentAlert").html(
                        return_data.is_present + " telah hadir di presensi ini"
                    );
                }
                refreshCurrentUrl();
            } else {
...
function promptDeleteAndPresent(id, presence_id = null, santri_id = null) {
    if (confirm("Apakah anda yakin untuk menghadirkan dari perijinan ini ?")) {
        ...
        $.get(
            base_url + `/presensi/izin/pengajuan/delete_and_present`,
            datax,
            function (data, status) {
                var return_data = JSON.parse(data);
                if (return_data.status) {
...
function actionSavePermit(action) {
        ...
        if (el[i].checked) {
            if (action == "reject") {
                ...
                if (alasan == "") {
                    $("#alertModal").fadeIn();
                    $("#contentAlert").html(
                        "Berikan alasan pada form yang masih kosong"
                    );
                    return false;
                }
            }
    ...
    for (var i = 0; i < el_berjangka.length; i++) {
        if (el_berjangka[i].checked) {
            ...
            ival_berjangka.push([
                el_berjangka[i].getAttribute("presence-id"),
                el_berjangka[i].getAttribute("santri-id"),
                alasan,
            ]);
        }
    }
    if (ival.length + ival_berjangka.length == 0) {
        $("#alertModal").fadeIn();
        $("#contentAlert").html(
            "Silahkan pilih minimal satu mahasiswa / perijinan!"
        );
        return false;
    } else {
        $("#loadingSubmit").show();
        if (ival.length > 0) {
            ...
            datax["action"] = action;
            datax["data_json"] = JSON.stringify(ival);
            $.get(base_url + `` + url + action, datax, function (data, status) {
                var return_data = JSON.parse(data);
                if (return_data.status) {
                    if (return_data.is_present != "" && action == "approve") {
                        $("#alertModal").fadeIn();
                        $("#contentAlert").html(
                            return_data.is_present +
                                " telah hadir di presensi ini"
                        );
                    }
                    refreshCurrentUrl();
                }
        ...
        if (ival_berjangka.length > 0) {
            ...
            if (action == "delete") {
                $("#alertModal").fadeIn();
                $("#contentAlert").html(
                    "Perijinan berjangka tidak dapat dihapus"
                );
                return false;
            } else if (action == "approve") {
                pesan_action = "menyetujui";
            ...
            $.get(
                base_url + `/presensi/izin/pengajuan/berjangka/` + action,
                datax,
                function (data, status) {
                    var return_data = JSON.parse(data);
                    if (return_data.status) {
...
function getMateri(santri_id, nama) {
    ...
    $.post(
        base_url + `/materi/monitoring/materi_santri`,
        {
            santri_id: santri_id,
        },
        function (data) {
...
function searchDataSantri(id, value) {
    var santris = document.querySelectorAll(
        "#" + id + " table tbody tr td span.santri-name"
    );
