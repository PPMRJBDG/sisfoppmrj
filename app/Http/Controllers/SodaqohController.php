...
Adapun kekurangannya masih senilai: *Rp ' . number_format($nominal_kekurangan, 0) . ',-*';
                    ...
                    if ($request->input('info-wa') == "true") {
                        ...
                        if ($nohp != '') {
                            ...
                            if ($wa_phone != null) {
                                $caption = $status_lunas . ' Pembayaran Sodaqoh Tahunan PPM RJ Periode ' . $check->periode . ' an. *' . $check->santri->user->fullname . '* senilai *Rp ' . number_format($request->input('nominal_bayar'), 0) . '* sudah dikonfirmasi.
' . $history_payment . '
' . $text_kekurangan;
